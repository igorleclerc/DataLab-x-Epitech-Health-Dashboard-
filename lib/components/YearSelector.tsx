    ...
    <div className="relative">
      ...
      >
        ...
        <div className="flex-1 text-left min-w-0">
          <div className="font-semibold text-gray-900 truncate">{selectedOption?.label}</div>
          <div className="text-xs text-gray-500 mt-0.5 truncate hidden sm:block">{selectedOption?.description}</div>
        </div>
        
        <ChevronDown className={`w-5 h-5 ml-2 text-gray-400 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {/* Dropdown condensé - responsive */}
      {isOpen && (
        <div className="absolute right-0 left-0 sm:left-auto mt-3 w-full sm:w-72 bg-white border border-gray-200 rounded-2xl shadow-2xl z-[60] overflow-hidden max-h-[80vh] overflow-y-auto">
          {/* Header condensé */}
